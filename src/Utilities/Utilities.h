...
public:
    ...
    sf::Font *GetFont(const vector<Font *> &fonts, const uint8_t fontID);
    sf::Texture *GetTexture(const vector<AllTextures *> &textures, const uint8_t textureID);

    AnimTextureCombined GetAnim(const vector<Anim *> &anim, const uint8_t animID);
    sf::Texture *GetAnimTexture(const vector<Anim *> &anim,
